...
{
    internal class DepthMaps : FullScreenCustomPass
    {
        ...
        protected override void Execute(CustomPassContext ctx)
        {
            ...
            if (Helpers.isMainG3DCamera(camera) == false)
            {
                return;
            }
            ...
            for (int i = 0; i < internalCameraCount; i++)
            {
                ...
                bakingCamera.aspect = Mathf.Max(
                    bakingCamera.aspect,
                    indivDepthTextures[i].referenceSize.x
                        / (float)indivDepthTextures[i].referenceSize.y
                );
