...
3. Set the Api Compatibility Level to ".NET Framework" in `Edit -> Project Settings -> Player -> Other Settings -> Api Compatibility Level`.
4. Add the "G3DCamera" script to your main camera. The script can be found in the "Scripts" folder of the package.
5. Set the calibration and config path to the folder containing your calibration and config files. (This step can be skipped if you use the default calibration without a head tracking camera.)
6. Set the config file name to match the config file you want to use. (This step can be skipped if you use the default calibration without a head tracking camera.)
7. You are done with the installation. Next you have to match your scene to the autostereo display.

### Basic usage
The plugin is written to work with real life units. i.e. the eye separation is in meters and should correspond to the real life eye separation (roughly 6 centimeter).
